...
def _candidate_scanner_paths():
    """Return a list of common locations to look for scanner output folders."""
    home = os.path.expanduser('~')
    candidates = []
    if home:
        candidates.extend([
            os.path.join(home, 'Documents', 'Scans'),
            os.path.join(home, 'Desktop', 'Scans'),
            os.path.join(home, 'Pictures', 'Scans'),
            os.path.join(home, 'Downloads'),
            os.path.join(home, 'Scans'),
        ])
    candidates.extend([
        ...
        r'C:\Scans',
        r'D:\Scans',
        r'C:\Users\Public\Pictures\Scans',
        r'C:\Users\Public\Documents\Scans',
    ])
...
def get_scanner_path():
    """
    Read the configured scanner watch-folder path.
    Falls back to the SCANNER_PATH environment variable, then a few
    common scanner directories if present.
    """
    env_path = os.getenv('SCANNER_PATH', '').strip()
    if env_path:
    ...
    for candidate in _candidate_scanner_paths():
...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        if not session.get('logged_in'):
            # Return JSON for AJAX requests so the frontend can handle auth failures cleanly.
...
def scan_from_folder(customer_id):
    ...
    if not scanner_path:
        return jsonify(
            ok      = False,
            message = 'Scanner folder not configured or no scanner folder was found. '
                      'Please set the scanner path in Settings, or place a scanned file in a common scan folder.',
...
# ─── Regular form upload (AJAX — returns JSON) ────────────────────────────────
# FIXED: was returning redirect() which caused infinite loading spinner.
# Now returns JSON so the JS fetch() call gets a proper response.
...
def upload_document(customer_id):
    ...
    # Guard: no file sent at all
    if not f or f.filename == '':
        return jsonify(ok=False, message='No file selected. Please choose a file.'), 400

    try:
        doc = _save_file_object(f, f.filename, customer_id)
        db.session.commit()
        return jsonify(
            ok       = True,
            message  = f'"{doc.original_name}" uploaded successfully!',
